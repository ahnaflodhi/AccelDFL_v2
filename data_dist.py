...
def data_iid(dataset, num_classes, num_nodes, val = False):
    """
...
def data_noniid(dataset, num_labels:int, num_users:int, alpha:float, idxlist = []) -> dict:
    """
    ...
    else: # Added if a list of validation indices as idxlist has been provided to extract from the main dataset
        class_ids = []
        for y in range(10):
            temp = []
            for idx in idxlist :
